...
import { applyFilters, personFilters } from "@/lib/filters";
...
export async function GET(request: Request) {
  try {
    ...
    data = applyFilters(data, searchParams, personFilters);
    ...
    if (sortParams) {
      data = sortBy(
        data,
        sortParams.field as keyof (typeof people)[0],
        sortParams.order,
      );
    } else {
    ...
    const results = paged.results
      .map((p) => getPersonFull(p.entityId))
    ...
    if (expandParam) {
      ...
      const expanded = results.map((person) =>
        expandEntity(person, "person", limitedExpand),
      );
    ...
    console.error("Error in GET /api/v1/people:", error);
