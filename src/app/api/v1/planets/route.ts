...
import { applyFilters, planetFilters } from "@/lib/filters";
...
export async function GET(request: Request) {
  try {
    ...
    data = applyFilters(data, searchParams, planetFilters);
    ...
    if (sortParams) {
      data = sortBy(
        data,
        sortParams.field as keyof (typeof planets)[0],
        sortParams.order,
      );
    } else {
    ...
    const results = paged.results
      .map((p) => getPlanetFull(p.entityId))
    ...
    if (expandParam) {
      ...
      const expanded = results.map((planet) =>
        expandEntity(planet, "planet", limitedExpand),
      );
    ...
    console.error("Error in GET /api/v1/planets:", error);
